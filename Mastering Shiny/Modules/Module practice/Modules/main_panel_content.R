...
main_panel_content_server <- function(id, selected_species = NULL, location_species = NULL, button_trigger = NULL, species_df = NULL, world = NULL){
  moduleServer(id, function(input, output, session){
    
    graph_title_event <- eventReactive(button_trigger(), {
      paste0("Sites where ", selected_species(), " has been observed")
    }, ignoreNULL = F)
    ...
    species_plot <- eventReactive(button_trigger(), {
      plot_species(selected_species(), location_species(), species_df, world)
    }, ignoreNULL = F)
